...
const getArticuloByNombre = async (nombre) => {
    try {
        const [rows] = await db.query("SELECT * FROM articulos WHERE nombre = ?", [nombre]);
        return rows[0];
    } catch (error) {
...
const createVenta = async (nroVenta) => {
    try {
        const [result] = await db.query("INSERT INTO venta (nro_venta) VALUES (?)", [nroVenta]);
...
const createDetalleVenta = async (ventaId, detalles) => {
    try {
        const values = detalles.map(({ articulo_id, precio, cantidad }) => [ventaId, articulo_id, precio, cantidad]);
